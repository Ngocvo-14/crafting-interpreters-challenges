...
class Parser {
    ...
    List<Stmt.Function> classMethods = new ArrayList<>();  // NEW

    while (!check(TokenType.RIGHT_BRACE) && !isAtEnd()) {
        // NEW: 'class' keyword before a method = static method
    ...
    return new Stmt.Class(name, superclass, methods, classMethods, mixins);
}
    ...
    // ── Challenge 3: break statement parsing ──────────────────────────────
    /**
     * breakStmt → "break" ";" ;
     *
     * It is a COMPILE-TIME (parse-time) error to use break outside a loop.
     * We track this with loopDepth: it is incremented when entering any loop
     * body (while or for) and decremented on exit. If loopDepth == 0 here,
     * the break is not enclosed by any loop.
     */
    ...
    private Stmt forStatement() {
        ...
        // ── Challenge 3: count this as an enclosing loop ───────────────────
        loopDepth++;
        Stmt body = statement();
        loopDepth--;
        ...
        // Desugar: wrap with while
        if (condition == null) condition = new Expr.Literal(true);
        body = new Stmt.While(condition, body);
        // Desugar: wrap with initializer block
        if (initializer != null) {
