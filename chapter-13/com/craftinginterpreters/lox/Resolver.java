...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  ...
  currentFunction = enclosingFunction;
    ...
    // Challenge 3: Break has no sub-expressions to resolve
    @Override
...
public Void visitClassStmt(Stmt.Class stmt) {
    ...
    // Your existing superclass checks — UNCHANGED
	...
	scopes.peek().put("inner_class", true);
	scopes.peek().put("inner_method", true);
    }

    // Instance methods — inside 'this' scope (UNCHANGED)
    beginScope();
    scopes.peek().put("this", true);
    ...
    // NEW: Static methods resolved OUTSIDE the 'this' scope
    // so that using 'this' inside a static method is a resolver error
    for (Stmt.Function method : stmt.classMethods) {
        resolveFunction(method, FunctionType.METHOD);
    }

    if (stmt.superclass != null) endScope(); // closes 'super' scope
...
public Void visitInnerExpr(Expr.Inner expr) {
    ...
    // Resolve to the same scope where inner_class is defined
    resolveLocal(expr, new Token(TokenType.IDENTIFIER, "inner_class", null, expr.keyword.line));
    return null;
}
    @Override
