...
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
    ...
    private static class VarLocation {
    	final int depth;
    	final int index;
    	VarLocation(int depth, int index) {
       		this.depth = depth;
        	this.index = index;
   	 }
     }
    private final Map<Expr, VarLocation> locals = new HashMap<>();
    ...
    private Object lookUpVariable(Token name, Expr expr) {
    	VarLocation location = locals.get(expr);
    	if (location != null) {
    ...
    void resolve(Expr expr, int depth, int index) {
    	locals.put(expr, new VarLocation(depth, index));
    }
    ...
    // Create a Stmt.Function with null name to signal "anonymous"
    ...
    public Object visitSuperExpr(Expr.Super expr) {
    	VarLocation location = locals.get(expr);  // ← was: int distance = locals.get(expr)
    	LoxClass superclass = (LoxClass) environment.getAt(location.depth, "super");
    	LoxInstance object  = (LoxInstance) environment.getAt(location.depth - 1, "this");
