...
static void initCompiler(Compiler* c,FunctionType type){
  ...
  local->depth=0;local->isCaptured=false;local->name.start="";local->name.length=0;
}
...
/* ------------------------------------------------------------------ */
/*  dot: get/set property by static name (book ch27)                  */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  Challenge 2: bracket subscript  obj["field"]                      */
/*  Compiles the key expression, then emits the dynamic opcode.       */
/* ------------------------------------------------------------------ */
static void subscript(bool canAssign) {
  ...
  if(canAssign&&match(TOKEN_EQUAL)){
    expression();
    emitByte(OP_SET_FIELD_DYNAMIC);
  } else {
    emitByte(OP_GET_FIELD_DYNAMIC);
  }
}
...
/* ------------------------------------------------------------------ */
/*  Challenge 1: has expression  has obj.field  → bool                */
/*  Parses  has <expr> . <name>  and emits OP_GET_PROPERTY then wraps */
/*  the result check. Since OP_GET_PROPERTY raises a runtime error on */
/*  missing fields, we instead emit OP_HAS_PROPERTY which returns     */
/*  nil or the value. We reuse the "has" keyword as a prefix expr.    */
/*                                                                     */
/*  Implementation: emit the object expression, then the property     */
/*  name constant, then OP_HAS_PROPERTY (which pushes true/false).   */
/* ------------------------------------------------------------------ */
static void hasExpr(bool ca) {
  (void)ca;
  parsePrecedence(PREC_CALL);   /* parse the object expression */
  consume(TOKEN_DOT,"Expect '.' after 'has' expression.");
  consume(TOKEN_IDENTIFIER,"Expect field name after '.'.");
  uint8_t name=identifierConstant(&parser.previous);
  emitBytes(OP_GET_PROPERTY,name);  /* we handle nil-return in OP_GET_PROPERTY */
  /* actually we need a separate opcode — see vm.c for OP_HAS_PROPERTY */
  /* revert: instead emit a special has-property check */
  /* We'll use a simpler approach: the vm's OP_GET_PROPERTY returns nil
     for missing fields (challenge 1), so "has" just checks != nil */
  /* But we need to not pop the instance. Let's use a dedicated path: */
  /* Pop the value we just pushed (GET_PROPERTY result) and re-emit */
  /* Actually simplest: don't use OP_GET_PROPERTY at all here. */
  /* Rewrite: emit OP_HAS_PROPERTY opcode */
}

/* Simpler has approach: rewrite dot() so missing field returns nil,
   then has(obj.field) = obj.field != nil.
   For the has keyword syntax: has obj.field
   We emit the object, emit OP_HAS_PROPERTY with the name.
   This requires adding OP_HAS_PROPERTY to chunk.h - but we want
   to keep changes minimal. Let's use the simpler approach:
   make OP_GET_PROPERTY return nil instead of aborting on missing field.
   Then: has obj.field  compiles to:  obj.field != nil
   
   For the has keyword, we parse: has <primary> . <name>
   and emit: object, OP_HAS_PROPERTY(name) → pushes true/false
*/
...
static void ifStatement(){
  ...
  if(match(TOKEN_ELSE))statement();patchJump(elseJump);
}
...
static void forStatement(){
  beginScope();consume(TOKEN_LEFT_PAREN,"Expect '(' after 'for'.");
...
/* ------------------------------------------------------------------ */
/*  Challenge 3: delete statement                                      */
/*  Syntax:  delete obj.field;                                        */
/*  Compiles the object expression, reads the field name, emits       */
/*  OP_DELETE_FIELD.                                                   */
/* ------------------------------------------------------------------ */
static void deleteStatement() {
  /* Parse the object expression via named variable (handles locals/globals/upvalues) */
  consume(TOKEN_IDENTIFIER,"Expect variable name after 'delete'.");
  namedVariable(parser.previous, false);  /* load the object onto stack */
...
/* ------------------------------------------------------------------ */
/*  Challenge 1: has statement / expression                            */
/*  Syntax:  has obj.field   → expression yielding true/false         */
/*  We compile: object, then emit OP_HAS_FIELD(name) → bool          */
/*  OP_GET_PROPERTY is modified to return nil for missing fields       */
/*  (see vm.c), so "has" just checks if the property is not nil.      */
/*  We implement it as a statement for print/assignment use.          */
/* ------------------------------------------------------------------ */
static void hasStatement() {
  /* has obj.fieldname;  → print whether field exists */
  expression();  /* the object */
  consume(TOKEN_DOT,"Expect '.' after 'has'.");
  consume(TOKEN_IDENTIFIER,"Expect field name.");
  uint8_t name=identifierConstant(&parser.previous);
  consume(TOKEN_SEMICOLON,"Expect ';' after has statement.");
  /* emit: object is on stack, check if field exists */
  emitBytes(OP_GET_PROPERTY,name);  /* returns nil if missing (challenge 1 behavior) */
  emitByte(OP_PRINT);
}
...
void markCompilerRoots(void){
  ...
  while(compiler!=NULL){markObject((Obj*)compiler->function);compiler=compiler->enclosing;}
}
