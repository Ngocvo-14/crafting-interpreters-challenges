...
static void emitReturn(){
  /* initializers return 'this' (slot 0) implicitly */
...
static void initCompiler(Compiler* c,FunctionType type){
  ...
  /* slot 0: 'this' for methods/initializers, empty for functions */
...
/* ch28: dot property access + OP_INVOKE optimization */
static void namedVariable(Token name,bool canAssign);
static void dot(bool canAssign) {
  consume(TOKEN_IDENTIFIER,"Expect property name after '.'.");
  uint8_t name=identifierConstant(&parser.previous);
  if(canAssign&&match(TOKEN_EQUAL)){
    expression();emitBytes(OP_SET_PROPERTY,name);
  } else if(match(TOKEN_LEFT_PAREN)){
    /* OP_INVOKE: fused get+call — the key optimization */
...
static void subscript(bool canAssign){
  expression();consume(TOKEN_RIGHT_BRACKET,"Expect ']' after subscript.");
...
static void returnStatement(){
  ...
  if(match(TOKEN_SEMICOLON)){emitReturn();}
    ...
    expression();consume(TOKEN_SEMICOLON,"Expect ';' after return value.");emitByte(OP_RETURN);
  }
}
...
/* ch28: compile a method declaration */
static void method(){
...
/* ch28: class declaration with methods */
