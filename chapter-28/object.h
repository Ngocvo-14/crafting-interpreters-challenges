...
/* ------------------------------------------------------------------ */
/*  ObjClass                                                            */
/*  Challenge 1: add ObjClosure* initMethod cache                     */
/*  Instead of looking up "init" in the hash table every time a class  */
/*  is instantiated, we store the init closure directly in the class  */
/*  object when the method is defined. Lookup becomes a NULL-check +  */
/*  pointer dereference — no hash table access at all.                */
/* ------------------------------------------------------------------ */
typedef struct {
  Obj        obj;
  ObjString* name;
  Table      methods;
  /* Challenge 1: cached pointer to the init() method, or NULL */
  ObjClosure* initMethod;
} ObjClass;
