...
/*
 *  * string() — Challenge 2 applied to the compiler
 *   *
 *    * Instead of always copying the string onto the heap (copyString),
 *     * we create a constant (non-owning) ObjString that points into the
 *      * source buffer directly.  The source buffer lives for the entire
 *       * duration of interpret(), so this is safe.
 *        *
 *         * This saves one heap allocation + memcpy per string literal.
 *          */
static void string(void) {
	  /* +1 / -2 trim the surrounding quote characters */
...
static void grouping(void) { expression(); consume(TOKEN_RIGHT_PAREN,"Expect ')' after expression."); }
	  ...
	  advance();
	    ParseFn prefix=getRule(parser.previous.type)->prefix;
		  ...
		  while(precedence<=getRule(parser.current.type)->precedence) {
			      advance(); getRule(parser.previous.type)->infix();
			        }
}
