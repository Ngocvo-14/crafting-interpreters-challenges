 ...
 *
 *   Challenge 1 – Conditional execution via dynamic dispatch (no if statement)
 *     Demonstrated in Lox code (see test file). The interpreter already
 *     supports everything needed: first-class functions (LoxFunction) and
 *     dynamic dispatch via class method lookup (LoxInstance.get → LoxFunction.bind).
 *     The Lox classes Bool, True, and False show how you replicate if/else purely
 *     through message passing, exactly like Smalltalk.
 *
 *   Challenge 2 – Looping via recursion (no while/for statement)
 *     Same support. Recursive Lox functions work because every call returns
 *     properly. True TCO is NOT implemented here (Java doesn't support it
 *     natively), so deep recursion will overflow the Java stack. The test uses
 *     a small n so it completes without issue. A production implementation
 *     would convert tail calls to iteration (trampoline or explicit loop).
 *
 *   Challenge 3 – break statement
 *     visitBreakStmt() throws BreakException.
 *     visitWhileStmt() catches BreakException to exit the loop.
 *     The parser already rejected break outside a loop at parse time.
 */
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
    ...
    // ── Challenge 3: break ─────────────────────────────────────────────────
    /**
     * Throw a BreakException to unwind to the nearest enclosing while loop.
     * The exception propagates transparently through blocks, if-statements,
     * function calls — wait, NOT function calls. A break inside a function
     * defined inside a loop would be a parse error (loopDepth would be 0
     * inside the function's parser context). So by the time we get here,
     * we are guaranteed to be lexically inside a while body.
     */
    @Override
    ...
    // ── Challenge 3: while catches BreakException ──────────────────────────
    /**
     * The only change from the base chapter is the try/catch block.
     * When a BreakException arrives, we simply stop iterating — the catch
     * body is intentionally empty. Execution continues normally after
     * the while loop.
     *
     * Because for-loops desugar to while-loops, break works inside for-loops
     * too with zero additional code.
     */
    @Override
