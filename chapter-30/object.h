...
struct Obj { uint32_t typeAndMark; Obj* next; };
...
typedef struct ObjUpvalue { Obj obj; Value* location; Value closed; struct ObjUpvalue* next; } ObjUpvalue;
typedef struct { Obj obj; ObjFunction* function; ObjUpvalue** upvalues; int upvalueCount; } ObjClosure;

/* ----------------------------------------------------------------
   ObjClass: ch29 adds superclass pointer.

   Challenge 1: superclass pointer enables runtime field-conflict
   detection in init() — we walk the chain at init time and warn
   if any ancestor already uses the same field name.

   Challenge 3: superclass chain is walked by OP_INNER to find the
   next subclass that defines the same method.
---------------------------------------------------------------- */
typedef struct ObjClass {
  Obj              obj;
  ObjString*       name;
  Table            methods;
  ObjClosure*      initMethod;   /* ch28 cached init */
  struct ObjClass* superclass;   /* ch29: NULL for root classes */
} ObjClass;
...
static inline bool isObjType(Value v,ObjType t){return IS_OBJ(v)&&OBJ_GET_TYPE(AS_OBJ(v))==t;}
